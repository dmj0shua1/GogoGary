...
public class ieCurrentLevelZoomIn : MonoBehaviour {
    ...
    void Awake() 
    {
        if (PlayerPrefs.GetInt("OpenStageIceAge") == 1)
        {
            Isactivate = true;
        }
    }
    ...
    void Update() 
    {
        if (Isactivate)
        {
            SampleButton = GameObject.Find("ieBuilding_L" + UnlockLevelHolder).GetComponent<Button>();
